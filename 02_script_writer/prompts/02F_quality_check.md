...
3. 必须额外检查 8 个增强点：多版本选择、分集/分段、情绪曲线、口播节奏、画面可执行性、角色上场人数、单帧连续性链表、失败样本回灌建议。
4. 如果需要重跑，必须给出 retry_stages，只能使用：02A、02B、02C、02D、02E。
5. retry_stages 应从最早出问题的阶段开始。例如分集策略错，应重跑 02A；结构压缩过狠，应重跑 02B；语音行超过 12 秒，应重跑 02C；正文没有按语音行写，应重跑 02D；单帧动作链不足，应重跑 02E。
6. revision_instructions 必须具体，能直接传回对应阶段让 LLM 修改。
7. 不得在本阶段直接重写完整剧本；只能做质量报告、证据、修改计划。
8. 当前后续采用单帧分镜：单帧是生产单位，四宫格只作为后续连续性预览/检查单位。02F 必须检查 visual_dramatic_units 和 continuity_chain 是否支持单帧连续性。
...
输出 JSON schema：
{
  ...
  "evidence_index": [
    {
      "evidence_id": "evd_001",
      "source": "01_event/01_golden_line/02_voice_line/02_segment/02_scene_beat/02_visual_unit/02_continuity_chain",
      "source_id": "对应 id",
      "finding": "发现的问题或通过原因"
    }
  ],
  ...
  "failure_learning_notes": [
    {
      "failure_type": "over_compression/dialogue_weak/os_overuse/voice_line_too_long/visual_abstract/too_many_characters/continuity_weak/source_line_lost/character_name_drift",
