...
输出 JSON schema：
{
  ...
  "adaptation_blueprint": {
    ...
    "target_format": "dialogue_os_blank",
  ...
  "compression_guardrails": {
    ...
    "rule": "原文信息量较大时，宁可增加段落，也不能强行压成 2 分钟。",
    "forbidden": ["多个关键事件合并成一句话", "只用 OS 概括冲突", "删除打脸/逼问/反杀节点"],
    "minimum_beat_principle": "每个关键事件至少对应一个 scene_beat 或一个连续 segment 组。"
  }
}
