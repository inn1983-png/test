...
function App() {
  ...
  async function taskAction(id: string, action: string) { await fetch(API + '/tasks/' + id + '/' + action, { method: 'POST' }); await load(); }
  ...
  const scripts = nodes.filter(n => n.type === 'script_block' || n.type === 'source_text');
  ...
  const content = useMemo(() => {
    ...
    if (page === 'Dashboard') return <Dashboard data={data} shots={shots} grids={grids} assets={assets} />;
    if (page === 'Script') return <ScriptPage scripts={scripts} setSelected={setSelected} />;
    ...
    if (page === 'Tasks') return <TaskPage tasks={data.tasks || []} taskAction={taskAction} />;
  ...
  }, [data, page, selected, settingsDraft, workflowDraft]);
...
function Dashboard({ data, shots, grids, assets }: any) { return <><section className="grid4"><Metric title="Shots" value={shots.length} /><Metric title="Grids" value={grids.length} /><Metric title="Assets" value={assets.length} /><Metric title="Tasks" value={(data.tasks || []).length} /></section><section className="panel"><h3>Canvas Snapshot</h3><pre>{JSON.stringify(data.canvas, null, 2)}</pre></section></>; }
function Metric({ title, value }: any) { return <div className="metric"><span>{title}</span><strong>{value}</strong></div>; }
function ScriptPage({ scripts, setSelected }: any) { return <section className="panel"><h3>Script Editor</h3>{scripts.map((n: any) => <div className="row" key={n.id} onClick={() => setSelected(n)}><span>{n.id}</span><span>{n.title || n.type}</span><Badge value={n.status} /></div>)}</section>; }
